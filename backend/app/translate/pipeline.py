...
class TranslationManager:
    ...
    async def submit(
        ...
        *,
        ...
        for block in blocks:
            ...
            elif row is not None and row.status in {"done", "failed"}:
        ...
        async with self._condition:
            ...
            candidates = sorted(
                (
                    ...
                    and abs(work.block.order_idx - first.block.order_idx) <= self.config.batch_size
                ),
                key=lambda work: (abs(work.block.order_idx - first.block.order_idx), work.block.order_idx),
            )
            selected = [first]
            for candidate in candidates:
                if candidate is first or len(selected) >= self.config.batch_size:
                    continue
                orders = [item.block.order_idx for item in selected] + [candidate.block.order_idx]
                if max(orders) - min(orders) <= self.config.batch_size - 1:
    ...
    async def _process_batch(self, batch: list[WorkItem]) -> None:
        ...
        try:
            ...
            result = await self._call_batch(paper, blocks, batch, glossary)
        except Exception as exc:
        ...
        for work in batch:
            ...
            if ratio < self.config.chinese_ratio_threshold:
                await self._complete_failure(
                    work,
                    f"LANGUAGE_DRIFT: chinese_ratio={ratio:.4f}, threshold={self.config.chinese_ratio_threshold:.4f}",
                )
            else:
                await self._complete_success(work, text, glossary[0].version if glossary else 1)
    ...
    async def _retry_drift(
        ...
        return result.translations[0].zh_text

    async def _complete_success(self, work: WorkItem, text_value: str, glossary_version: int) -> None:
